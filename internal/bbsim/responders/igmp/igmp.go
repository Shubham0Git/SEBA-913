...
func HandleNextPacket(onuId uint32, ponPortId uint32, serialNumber string, portNo uint32, onuStateMachine *fsm.FSM, pkt gopacket.Packet, stream openolt.Openolt_EnableIndicationServer, client openolt.OpenoltClient) {
        log.WithFields(log.Fields{
                ...
                "PortNo": portNo,
    }).Debugf("Entered HandleNextPacket")

    igmp, igmpErr := extractIGMP(pkt)

    if igmpErr != nil {
               log.Fatalf("Failed to Extract IGMP: %v", igmpErr)
               return
    }

    if igmp != nil {
       if(igmp.Version == 3) {
           log.Infof("Recieved Version 3 IGMP-------")
       }
    }

}

func extractIGMP(pkt gopacket.Packet) (*layers.IGMP, error) {
       layerIGMP := pkt.Layer(layers.LayerTypeIGMP)
       igmp, _ := layerIGMP.(*layers.IGMP)
       if igmp == nil {
               return nil, errors.New("Cannot extract IGMP")
       }
       return igmp, nil
}
